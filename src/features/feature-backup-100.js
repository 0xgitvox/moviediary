// Backup handling for MovieDiary.
const { EventEmitter } = require('node:events');

class BackupHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'backup' + ' payload');
 return { 'backup': String(payload).slice(0, 200) };
 }
...
module.exports = { BackupHandler, run: (payload, opts) => new BackupHandler().run(payload, opts) };